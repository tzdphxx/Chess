...
public class UserServlet extends BaseServlet{
    ...
    // 修改用户信息
    public void updateInfo(HttpServletRequest req, HttpServletResponse resp) throws IOException, SQLException, NoSuchFieldException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        HttpSession session = req.getSession(false);
        ...
        if (newPassword != null && !newPassword.trim().isEmpty()) {
            user.setPassword(PasswordUtil.encodedPassword(newPassword));
            changed = true;
            msg.append("密码已修改;");
        }
        // 修改手机号
        if (newPhone != null && !newPhone.trim().isEmpty() && !newPhone.equals(user.getPhone())) {
            // 检查手机号是否已被注册
            User exist = userService.selectByPhone(newPhone);
            if (exist != null && exist.getUserId() != user.getUserId()) {
                resp.getWriter().write("{\"success\":false,\"message\":\"手机号已被注册\"}");
                return;
            }
        ...
        if (newEmail != null && !newEmail.trim().isEmpty() && !newEmail.equals(user.getEmail())) {
            // 检查邮箱是否已被注册
            User exist = userService.selectByEmail(newEmail);
            if (exist != null && exist.getUserId() != user.getUserId()) {
                resp.getWriter().write("{\"success\":false,\"message\":\"邮箱已被注册\"}");
                return;
            }
        ...
        if (!changed) {
            resp.getWriter().write("{\"success\":false,\"message\":\"没有需要修改的信息\"}");
            return;
        }

        int result = userService.updateUser(user);
        if (result > 0) {
            resp.getWriter().write("{\"success\":true,\"message\":\"" + msg.toString() + "\"}");
        } else {
            resp.getWriter().write("{\"success\":false,\"message\":\"修改失败\"}");
        }
    }
}
